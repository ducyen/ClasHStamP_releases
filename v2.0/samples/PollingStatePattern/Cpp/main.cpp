...
#include "Model/AirspeedSensor.hpp"
...
int main() {
	...
	AirspeedSensor context(0);
    for (int i = 0; i < (int)AirspeedSensor::_EventId::evNum; i++) {
        std::cout << "Input " << i << " for " << context.EventId_toString((AirspeedSensor::_EventId)i) << std::endl;
    }
	int n;
	do {
	    context.Start();
